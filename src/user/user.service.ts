...
export class UserService {
  constructor(@InjectModel(User.name) readonly userModal: Model<UserDocument>) {}
  ...
  async findById(id: Types.ObjectId) {
    const user = await this.userModal.findById(id).select(USER_KEYS).lean().exec();
    return user;
  }

    async findByEmail({email,password}) {
    const user = await this.userModal.findOne({email,password}).select(USER_KEYS).lean().exec();
    return user;
  }
